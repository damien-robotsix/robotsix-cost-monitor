...
describe("render", () => {
  ...
  it("renders placeholder with detail message when run has detail", () => {
    ...
    expect(document.getElementById("run-meta").innerHTML).toContain(
  ...
  it("renders empty traces placeholder", () => {
    ...
    expect(document.getElementById("analyzed").innerHTML).toContain(
      "no traces were analyzed",
    );
  });
  ...
  it("renders empty proposals placeholder", () => {
    ...
    expect(document.getElementById("proposals").innerHTML).toContain(
      "no proposals",
    );
  });
...
describe("renderTargeted", () => {
  it("renders not-run-yet message for falsy run", () => {
    ...
    expect(document.getElementById("target").innerHTML).toContain(
      "not run yet",
    );
  });
  ...
  it("renders full targeted run with header, summary, proposals, filing", () => {
    ...
    const run = {
      ...
      proposals: [
        { title: "fix loop", estimated_saving: "$30", rationale: "remove retry" },
      ],
...
describe("proposalsHTML", () => {
  ...
  it("renders proposal items", () => {
    const html = proposalsHTML([
      { title: "p1", estimated_saving: "$10", rationale: "r1" },
    ]);
